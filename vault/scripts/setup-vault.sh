...
vault secrets enable kv || ([ $? -eq 2 ] && exit 0)
...
vault auth enable jwt && sleep 1 || ([ $? -eq 2 ] && exit 0)

vault write auth/jwt/config \
    oidc_discovery_url="https://agent.buildkite.com" \
    oidc_client_id="" \
    oidc_client_secret="" || ([ $? -eq 2 ] && exit 0)

# create the policy that will be scoped to the token requested by the agent role
cat ./vault/config/agent-policy.hcl | vault policy write buildkite - || ([ $? -eq 2 ] && exit 0)

sleep 1

# Create our agent roles, one for approle and another for jwt and attach the agent policies created earlier
vault write auth/approle/role/buildkite token_policies="buildkite" token_ttl=1h token_max_ttl=4h && sleep 1 || ([ $? -eq 2 ] && exit 0)
